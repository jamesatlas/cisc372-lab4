...
#define COMM_STATE_SEND 0
#define COMM_STATE_WAITING 1
#define COMM_STATE_RECV 2
#define COMM_STATE_IDLE 3
...
public:
    ...
    int best_solution;
    int queue_position;
    MPI_Request last_request;
    int comm_state;
    int request_ida_moves;

    vector<MPI_Request> outgoing_sends;
    vector<int *> outgoing_buffers;
    ...
    void getWork() {
        ...
        if (s <= workunits_threshold) {
            bool blocking = (s == 0);
            if (comm_state == COMM_STATE_IDLE) {
                request_ida_moves = ida_moves;
                MPI_Isend(&request_ida_moves, 1, MPI_INT, 0, TAG_WORKUNIT,
                        MPI_COMM_WORLD, &last_request);
                comm_state = COMM_STATE_SEND;
            }
            if (comm_state == COMM_STATE_SEND) {
                int flag;
                MPI_Test(&last_request, &flag, &status);
                if (flag) {
                    comm_state = COMM_STATE_WAITING;
                }
                else if (blocking) {
                    MPI_Wait(&last_request, &status);
                    comm_state = COMM_STATE_WAITING;
                }
            }
            if (comm_state == COMM_STATE_WAITING) {
                int flag;
                ...
                if (flag) {
                    if (status.MPI_TAG == TAG_TERMINATE) {
                        ...
                        // there is a message waiting to be received
                        MPI_Irecv(receiveBuffer, workunit_size*2,
                                    MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD,
                                               &last_request);
                        comm_state = COMM_STATE_RECV;
                    }
                }
            }
            if (comm_state == COMM_STATE_RECV) {
                int flag;
                MPI_Test(&last_request, &flag, &status);
                if (!flag && blocking) {
                    MPI_Wait(&last_request, &status);
...
//            if (blocking) {
//                DEBUG("process %d was blocking because s was %d, now s is %d\n", mpi_rank, s, workunit_trees.size());
//                getWork();
//            }
        }
    }
    ...
    void clearBuffers() {
        ...
        int s = outgoing_sends.size();
        for (int i = 0; i < s;) {
            MPI_Request r = outgoing_sends[i];
            int flag;
            MPI_Test(&r, &flag, &status);
            if (flag) {
                int * unused = outgoing_buffers[i];
                outgoing_buffers[i] = outgoing_buffers[s];
                outgoing_buffers.pop_back();
                outgoing_sends[i] = outgoing_sends[s];
                outgoing_sends.pop_back();
                delete [] unused;
                s--;
            }
            else {
    ...
    void sendWork(int rank) {
        int * buffer = new int[workunit_size*2];
        ...
        for (int i = 0; i < workunit_size; i++) {
            ...
            buffer[i*2] = queue_position;
            buffer[i*2+1] = ida_moves;
        ...
        MPI_Isend(buffer, workunit_size*2, MPI_INT, rank, TAG_WORKUNIT, MPI_COMM_WORLD, &request);

        outgoing_buffers.push_back(buffer);
        outgoing_sends.push_back(request);
    }
...
int main(int argc, char **argv) {
    if (argc < 5) {
        printf("%s requires a filename argument, a total number of work units for expansion (> p), a per work unit distribution #, and a threshold # for when new work is requested.\n", argv[0]);
        return 1;
    }
    ...
    process.workunits_threshold = atoi(argv[4]);
